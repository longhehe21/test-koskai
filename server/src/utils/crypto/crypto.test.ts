...
describe('crypto/aes', () => {
  ...
  it('encryptJson/decryptJson: preserve object structure', async () => {
    ...
    const obj = { hoTen: 'Nguyễn Văn A', ngaySinh: '01/01/2000', thuongTru: { tinh: 'Hà Nội', xa: 'Tây Hồ' } };
...
describe('crypto/kms', () => {
  ...
  it('throw với salt quá nhỏ', async () => {
    const { deriveCitizenKey } = await import('./kms.js');
    await expect(deriveCitizenKey('a'.repeat(64), Buffer.alloc(8))).rejects.toThrow();
  });
...
describe('end-to-end: encrypt PII vault như real flow', () => {
  it('hash CCCD → derive key → encrypt PII → decrypt lấy lại', async () => {
    ...
    const piiPayload = { hoTen: 'Nguyễn Văn A', cccd: cccdNumber, ngaySinh: '01/01/2000' };
